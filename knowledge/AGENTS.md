...
3. **Спецификация** до кода → `knowledge/decisions/`, с acceptance и failure modes (`spec-driven-workflow.md`). Ревью привязывается к hash версии спеки.
4. **Cross-agent review спеки — ≥2 НЕЗАВИСИМЫХ ревьюера, до человека.** Только материальные решения (архитектура/зависимость/миграция/security/публичный артефакт/методология/крупная ревизия). Ревьюеры исключают автора. Layer 1 (направление) → Layer 2 (детали). Вердикт структурный (`approve|approve-with-nits|block`, привязан к `spec:<algo>:<hash>`, обязательные поля — в `cross-agent-review.md`; без них ревью не засчитывается). `block` замораживает до re-review (отбить текстом нельзя; security-block снимает только человек); **гейт удовлетворён только вердиктами на точной финальной версии** — материальная правка инвалидирует прежние approve. Разногласие X-vs-Y → третий агент или человек. Ревьюер недоступен: первичное ревью → `[Single-Agent Fallback: <reason>]`; re-review после ≥2 логированных попыток → `[Blocker-Re-Review-Unavailable: ...]`. Не молчать.
5. **Тесты → имплементация.** Red → green → ревизии. Маленькие проверяемые шаги. Пустой/упавший вывод = провал, не «нет результата».
5.5. **Implementation review — диффа, не только спеки.** Нетривиальная работа с кодом/тестами/конфигами: после green дифф уходит ≥1 независимому ревьюеру (материальная → ≥2), не автору. Handoff: спека+hash, diff ref, тесты, acceptance, «в чём не уверен». Чеклист и формат вердикта (`spec:<algo>:<hash> diff:<ref>`) — в `cross-agent-review.md`.
...
7. **Verify + close.** Acceptance-матрица: каждый критерий → evidence (реальный вывод) → verifier → verdict. Материальная работа: верификатор = один из ревьюеров 5.5 (не автор), сам повторяет ключевые проверки; прочая — автор, но evidence воспроизводим любым агентом. Рантайм-поведение (installer/hook/scheduler/UX) → реальный smoke, не только unit/lint. Затем `kb-doctor` (нет свежих P0/P1) и `kb-board close ... --claim-id`. Evidence до заявления «готово».
...
**Инциденты учат цикл:** запись о сломавшейся отгруженной работе в `incidents.md` обязана содержать `loop-phase-failed: <0…8|5.5>`, почему гейты пропустили, prevention rule. Одна фаза фейлит дважды → правится сам цикл.

**Task-board триггер (enforcement для не-Claude):** перед переводом задачи в `In Progress` агент обязан убедиться, что спека есть в `knowledge/decisions/` и (для материальной работы) прошла ≥2-агентное ревью; иначе первая подзадача — написать спеку. `agy` запускать через `agy --add-dir "$PWD"`, чтобы он читал проектный `AGENTS.md`. Детерминированные guards (`kb-board`/`kb-doctor dev-loop-audit`) — отдельная tracked-задача; до их появления правила enforced инструкцией и владельцем.
